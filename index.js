...
async function animateText(text) {
    ...
    const interval = setInterval(() => {
        // Use a temporary variable to store the next character/segment
        const chunk = text.slice(i, i + 1);
        // Check for opening template literal syntax
        if (chunk === '${') {
            // Find the closing `}`
...
async function scene4() {
    ...
    await sleep();
    rainbowTitle.stop();
    await animateText(`With all the gingerbread cookies recovered, Jingle and Rani return to the mango tree. The Monkey Spirit uses them to reassemble the Naughty and Nice List, revealing a heartwarming surprise: every child in India, naughty or nice, deserves a Christmas miracle.
